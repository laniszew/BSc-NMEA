...
export const ParseCommonDegrees = (token: string): Number => {
    ...
    return round(((degree + minutes / 60 + seconds / 3600) * 1000 + Number.EPSILON)) / 1000;
};
...
export const parseIntSafe = (str: string, radix: number = 10): number => (str === '' ? 0 : parseInt(str, radix));
